...
const HOSTS: HostRow[] = [
  ...
  { name: "web-02.example.com", ip: "10.20.0.22", spec: "4C8G · 80G", os: "Debian 12", status: "running", cpu: 37, mem: 62, last: "2026-08-31 09:10" },
  { name: "db-01.example.com", ip: "10.20.0.31", spec: "8C16G · 200G", os: "Ubuntu 22.04", status: "maint", cpu: 12, mem: 83, last: "2026-08-30 22:40" },
  { name: "cache-01.example.com", ip: "10.20.0.41", spec: "2C8G · 40G", os: "Debian 12", status: "running", cpu: 8, mem: 34, last: "2026-08-31 09:11" },
  { name: "backup-01.example.com", ip: "10.20.0.51", spec: "2C4G · 500G", os: "Ubuntu 22.04", status: "stopped", cpu: 0, mem: 0, last: "2026-08-28 18:02" },
];
...
export default function OpsConsole({ simState, messages, onSend, busy }: SimProps) {
  ...
  const activeJob =
    busy && jobs.length > 0 && jobs[jobs.length - 1].output === null
      ? jobs[jobs.length - 1]
      : null;
  ...
  const hostsVisible = HOSTS.filter(
    (h) => !q || h.name.toLowerCase().includes(q) || h.ip.includes(q)
  );
  ...
  const navItem = (
    ...
    return (
      ...
      >
        ...
        {count != null && (
          <span className={`text-[11px] font-semibold ${active ? "text-blue-600" : "text-slate-400"}`}>
            {count}
          </span>
        )}
      </button>
      ...
      <div className="flex-1 flex min-h-0">
        <aside className="w-52 shrink-0 bg-white border-r border-slate-200 flex flex-col">
          <nav className="flex-1 overflow-y-auto product-scroll px-2 py-2.5 space-y-3">
            ...
            <div>
              <div className="px-2.5 pb-1 text-[10px] font-medium tracking-wider text-slate-400">
                资源管理
              </div>
                      ...
                      <tbody>
                        {hostsVisible.slice(0, 4).map((h) => (
                          ...
                          >
                            ...
                            <td className={TD}>
                              <PBadge tone={HOST_STATUS[h.status].tone}>
                                {HOST_STATUS[h.status].label}
                              </PBadge>
                            </td>
                            <td className={TD}>
                              <Meter value={h.cpu} />
            ...
            {tab === "files" && (
              <div className="p-5 space-y-4">
                {pageHead(
                  "工作目录",
                  <>
                    主机 ops-test · <span className="font-mono">{workdirTail}</span> · 只读 · 每 2
                    秒同步
                  </>,
                  <PButton variant="outline" icon="refresh">
                    同步目录
                ...
                <div className="flex items-start gap-2 rounded-lg border border-blue-100 bg-blue-50 px-3.5 py-2.5 text-[12px] text-blue-800">
                  ...
                  <span>
                    点「查看内容」让运维助手读取文件。新产生的脚本产物会标成「新增」，并在 2 秒内同步到列表。
                  </span>
                </div>
              </div>
            )}
