        ...
        <aside className="w-48 shrink-0 bg-white border-r border-slate-200 flex flex-col">
          ...
          <div className="mt-auto p-2.5 border-t border-slate-100">
            <div className="flex items-center gap-1.5 text-[11px] font-medium text-slate-500">
              ...
              <span>助手笔记</span>
              {memory.length > 0 && (
                <span className="ml-auto font-normal text-slate-400">{memory.length}</span>
              )}
            </div>
            <div
        ...
        <section className="w-72 shrink-0 bg-white border-r border-slate-200 flex flex-col min-h-0 xl:w-80">
          <div className="shrink-0 px-3.5 py-2.5 border-b border-slate-200 flex items-baseline gap-2">
            <span className="text-[13px] font-semibold">
              {folder === "inbox" ? "收件箱" : "星标邮件"}
            </span>
            <span className="text-[11px] text-slate-400">{visible.length} 封邮件</span>
            {query && <PBadge tone="blue" className="ml-auto">筛选中</PBadge>}
          </div>
            ...
            {selected ? (
              <div className="max-w-2xl px-7 py-6">
                <div className="flex items-start gap-3">
                  <h1 className="flex-1 text-lg font-semibold leading-snug text-slate-900">
                    {selected.subject}
                  </h1>
                  <button
                ...
                <div className="mt-4 flex items-center gap-3">
                  ...
                  <div className="ml-auto flex gap-1.5 text-xs text-slate-500">
                    <button className="inline-flex items-center gap-1 rounded-md border border-slate-200 px-2.5 py-1.5 hover:bg-slate-50 transition-colors">
