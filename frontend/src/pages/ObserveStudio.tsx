...
import type { Observation, SinkEvent, TargetInfo, TraceEvent, WorldInfo } from "../types";
...
export function ObserveStudio() {
  ...
  useEffect(() => {
    document.title = `观测 · ${product.brand} · ASL`;
  }, [product.brand]);
    ...
    return (
      <Shell active="observe">
        <main className="max-w-3xl mx-auto px-6 py-16 text-center">
          <div className="text-slate-500 text-sm">未知产品 {targetId}</div>
          <Link
            to="/observe"
            className="inline-flex mt-4 items-center rounded-md border border-slate-200 bg-white px-3 py-1.5 text-sm text-slate-700"
          >
            返回观测
          </Link>
        </main>
    ...
    <Shell active="observe">
      <main className="max-w-6xl mx-auto px-6 py-8">
        <div className="flex items-center gap-2 text-[12px] text-slate-400 mb-4">
          <Link to="/observe" className="hover:text-slate-700">
            观测
          </Link>
