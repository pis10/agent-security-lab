          ...
          <div className="grid grid-cols-3 gap-3">
            {sites.map((f) => {
              ...
              return (
                ...
                >
                  <span className={`h-9 w-9 rounded-full flex items-center justify-center ${m.chip}`}>
                    <Icon name={m.icon} size={16} />
                  </span>
                  <span className="text-xs font-medium text-slate-700 truncate max-w-full">{m.title}</span>
                  <span className="text-[10px] font-mono text-slate-400 truncate max-w-full">{f}</span>
                </button>
              );
            })}
          </div>
...
export default function FakeBrowser({ simState, messages, onSend, onAct, busy }: SimProps) {
  ...
  const [tabs, setTabs] = useState<TabState[]>([{ id: 1, stack: ["home.html"], idx: 0, reloadKey: 0 }]);
  ...
  const reload = () =>
    setTabs((prev) => prev.map((t) => (t.id === activeId ? { ...t, reloadKey: t.reloadKey + 1 } : t)));
  ...
  const lastAi =
    messages.length > 0 && messages[messages.length - 1].role === "assistant"
      ...
      : null;
  // biome-ignore lint/correctness/useExhaustiveDependencies: 视口跟随只需对“新回复”触发一次，navigate/currentFile 刻意不追踪
  useEffect(() => {
  ...
  return (
    <div className="h-full flex flex-col bg-[#dee1e6] text-slate-800 relative">
        ...
        <div className="flex-1 min-w-0 flex items-end gap-1 overflow-x-auto product-scroll">
          {tabs.map((t) => {
            ...
            return (
              ...
              >
                {file ? <Favicon file={file} /> : <Icon name="plus" size={10} className="text-slate-400 shrink-0" />}
                <span className="flex-1 truncate">{file ? metaOf(file).title : "新标签页"}</span>
                <button
                  type="button"
                  title="关闭标签页"
                  onClick={(e) => {
      ...
      <div className="shrink-0 flex items-center gap-1 px-2.5 py-1 bg-white border-b border-slate-200">
        ...
        <span className="text-[11px] text-slate-400 select-none">收藏夹栏 · 共 {sites.length} 个页面</span>
      </div>
          ...
          {historyOpen && (
            <div className="absolute top-2 right-3 z-10 w-80 p-card overflow-hidden">
              ...
              <div className="max-h-64 overflow-y-auto product-scroll">
                {log.length === 0 && <div className="p-6 text-center text-xs text-slate-400">暂无浏览历史</div>}
                {log.map((h, i) => (
                  <button
                    key={i}
                    type="button"
                  ...
                  >
                    ...
                    <span className="flex-1 min-w-0">
                      <span className="block truncate text-[13px] text-slate-700">{metaOf(h.file).title}</span>
                      <span className="block truncate font-mono text-[11px] text-slate-400">{urlOf(h.file)}</span>
                    </span>
                    {h.by === "ai" && (
