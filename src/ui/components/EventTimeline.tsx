...
const KIND_META: Record<string, { label: string; icon: string; dot: string; chip: string }> = {
  user_msg: {
    label: "你",
    icon: "user",
    dot: "bg-sky-500",
...
const timeOf = (ts: number) => new Date(ts * 1000).toLocaleTimeString("zh-CN", { hour12: false });
        ...
        {FILTERS.map((f) => (
          <button
            ...
            className={`px-2.5 py-1 rounded-md text-[12px] transition-colors ${
              filter === f.id ? "bg-slate-900 text-white" : "text-slate-500 hover:text-slate-800 hover:bg-slate-100"
            }`}
          >
            {f.label}
          </button>
