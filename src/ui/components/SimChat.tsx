...
const LIGHT_ACCENT: Record<ChatAccent, { user: string; send: string; avatar: string; pulse: string }> = {
  blue: {
  ...
  return (
    <div className={`flex flex-col h-full ${light ? "bg-white text-slate-800" : ""}`}>
      ...
      >
        ...
        {messages.map((m, i) => (
          <div key={i} className={`rise-in flex items-end gap-1.5 ${m.role === "user" ? "flex-row-reverse" : ""}`}>
            <span
        ...
        {busy && (
          <div className={`flex items-center gap-1.5 text-[12px] ${light ? "text-slate-400" : "text-dim font-mono"}`}>
